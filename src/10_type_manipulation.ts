...
type Userkeys = keyof User; // 아래와 똑같음
// type Userkeys = 'id' | 'name' | 'age';
// const key: Userkeys = 'email'; // Error (email없음)
...
function printAuthorInfo2(author: Post['author']): void { // 유지보수가 쉬워짐
    console.log(`${author.id}: ${author.name}`);
}

// 배열 요소의 타입추출
const COLORS = ['red', 'green', 'blue'] as const;
type Color = typeof COLORS[number]; // 배열의 값들을 유니온 타입으로 한번에 추출

// 튜플의 요소 타입 추출
type Tuple = [number, string, boolean];
type Tup1 = Tuple[0] // number
type Tup2 = Tuple[1] // string
type Tup3 = Tuple[number] // number | string | boolean
